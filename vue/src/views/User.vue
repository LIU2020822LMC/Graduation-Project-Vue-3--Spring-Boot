<template>
  ...
  <div class="card" style="margin-bottom: 5px">
    ...
    <el-upload
        ...
        action="http://localhost:2020/user/import"
...
<!--   @current-change="load"<---事件绑定，最终实现了分页功能的数据动态加载,这一行的意思就是每次点击下一页或者点击某一页就会刷新页面数据-->
    <el-dialog title="普通用户信息" v-model="data.formVisible"  width="500" destroy-on-close="true">
...
const exportData = () =>{
  ...
  //window.open("http://localhost:2020/user/export");//前端调用后端导出数据
  let url = `http://localhost:2020/user/export?username=${data.username === null ? '' : data.username}`
